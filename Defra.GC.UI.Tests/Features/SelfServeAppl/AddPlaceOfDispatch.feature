...
Scenario: Verify back link on Place Of Dispatch page
	...
	| test1C    | Tesco York         | Northern Ireland Retail Movement Scheme | Add a place of dispatch | You do not need to add an establishment that | testName11        | testAddress1 | London  | England    | SE10 9NF     | testName12         | testAddress2 | Liverpool | England     | L1 0AN        |
...
Scenario: Verify zero results page on Place Of Dispatch page
	...
	| test1C    | Tesco York         | Add a place of dispatch | SE9 1EE      | 0 results    |
...
Scenario: Verify error messages for place of dispatch mandatory fields
	...
	| test      |	 Tesco York        | SE10 9NF |                   | testAddress1 | testCity   | testCountry | SE10 9NF     | Enter an establishment name                      |
	| test      |    Tesco York		   | SE10 9NF | testErrorName1    |              | testCity   | testCountry | SE10 9NF     | Enter address line 1                             |
	| test      |    Tesco York        | SE10 9NF | testErrorName2    | testAddress1 |            | testCountry | SE10 9NF     | Enter a town or city                             |
	| test      |    Tesco York		   | SE10 9NF | testErrorName3    | testAddress1 | testCity   | testCountry |              | Enter a postcode                                 |
	| test      |    Tesco York        | SE10 9NF | testErrorName4$%  | testAddress1 | testCity   | testCountry | SE10 9NF     | Enter an establishment name using only letters, numbers, brackets, full stops, hyphens, underscores, forward slashes, apostrophes or ampersands |
	| test      |    Tesco York		   | SE10 9NF | testErrorName5    | testAddr%$   | testCity   | testCountry | SE10 9NF     | Enter address line 1 using only letters, numbers, brackets, full stops, hyphens, underscores, forward slashes, apostrophes or ampersands     |
	| test      |    Tesco York        | SE10 9NF | testErrorName6    | testAddress1 | testCity%$ | testCountry | SE10 9NF     | Enter a town or city using only letters, numbers, brackets, full stops, hyphens, underscores, forward slashes, apostrophes or ampersands     |
	| test      |    Tesco York        | SE10 9NF | testErrorName7    | testAddress1 | testCity   | testCountry | testCode$%   | Enter a real postcode                            |
...
Scenario: Verify error message for blank Establishment postcode field & non GB post code
	...
	| test1A    | Tesco York         | BT93 8AD  | Enter a postcode in England, Scotland or Wales |