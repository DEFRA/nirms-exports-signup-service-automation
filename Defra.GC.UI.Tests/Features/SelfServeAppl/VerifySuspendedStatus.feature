...
Scenario: Verify Suspended status 
	...
	Then  verify status '<Status>' on self serve business dashboard page
	...
	Examples: 
	| logininfo | Business selection | Business RMS Number | nextPage                                |Status			  |
	| test1A    | TestEnv2           | RMS-GB              | Northern Ireland Retail Movement Scheme |Suspended from NIRMS|
